...
* Outbound run:
  1. sense `front/left/right` walls via IR at each cell center
  2. update maze map
  3. run BFS flood-fill to the center
  4. choose the neighbor with smallest distance (tie-break: forward, left, right, back)
* After first reaching the goal:
  1. keep the learned maze knowledge
  2. switch target to the start cell `(0,0)`
  3. run BFS again to find the shortest return path
  4. use only known passages on the return leg
* The run ends only after the mouse comes back to the start cell and brakes there.

Unknown edges are treated as open while exploring toward the goal. On the return leg, only known passages are allowed.
